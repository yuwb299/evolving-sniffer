...
class PacketStatistics:
    ...
    def update(self, packet: Packet):
        ...
        if packet.ethernet:
            ...
            # (assuming Ethernet is the base for our Phase 1 implementation)
    ...
    def get_report(self) -> str:
        ...
        if self.ethernet_frames > 0:
            ip_percent = (self.ipv4_packets / self.ethernet_frames) * 100
            lines.append(f"  -> IPv4 Packets     : {self.ipv4_packets} ({ip_percent:.1f}%)")
            
            if self.ipv4_packets > 0:
                tcp_percent = (self.tcp_packets / self.ipv4_packets) * 100
                udp_percent = (self.udp_packets / self.ipv4_packets) * 100
